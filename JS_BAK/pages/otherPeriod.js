        ...
        if (l > 0) {
            ...
            $(data.otherPeriods).each(function () {
                ...
                if (this.status == "25" || this.status == "30" || this.status == "20") {
                    ...
                    html.push('<li class="qi_02" onclick="otherPeriod.GoToProductDetail(\'' + this.periodnum + '\',\'' + otherPeriod.GoodsId + '\')"><div class="qi_top"><label class="qi_num fl">第' + this.periodnum + '期</label><span class="qi_time fr">揭晓时间：' + this.openawardtime + '</span></div><div class="qi_describe"><p><label>幸运用户: 　</label><span>' + CutPhone(this.buyerphone) + '</span></p><p><label>幸运号码: 　</label><span class="qi_blue">' + this.awardcode + '</span></p><p><label>参与人次: 　</label><span>' + this.chipinnum + '</span></p><b class="qi_de_poi"></b></div></li>');
                    ++count;
                }
            });
            if (count > 0) {
