...
var MyOrder_detaile = {
    ...
    gotodetail: function (pid) {
        ...
        window.location.href = g_const_PageURL.Product_Detail + "?frd=yes&pid=" + pid + "&t=" + Math.random();
    },
...
function btncaozuo(btnname, order_code, order_money) {
    ...
    switch (btnname) {
        ...
        case "pjsd"://评价晒单
            PageUrlConfig.SetUrl();
            window.location.href = g_const_PageURL.ReviewRelease + "?frd=yes&order_code=" + order_code + "&s=d&t=" + Math.random();
            break;
    }
}
    ...
    GetList: function () {
        ...
        request.done(function (msg) {
            ...
            if (msg.resultCode == g_const_Success_Code) {
                ...
                //$("#sel_nextPage").val("1");
                //MyOrder_detaile.GetList();
                window.location.replace(g_const_PageURL.MyOrder_List + "?frd=yes&paytype=" + $("#hid_list_paytype").val());
            }
            else {
                ShowMesaage(msg.resultMessage);
            }
