...
//判断日期类型是否为YYYY-MM-DD hh:mm格式的类型 datetime
function isDateTimeMinute(obj) {
    if (obj.length != 0) {
...
//判断日期类型是否为YYYY-MM-DD hh:mm格式的类型 datetime或者null,空
function isDateTimeMinuteOrNull(obj) {
    var controlObj = $.trim(obj);
    if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
        return true;
    }
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            switch ($(this).attr("checkexpession")) {
                ...
                case "DateTime":
                    {
                        ...
                        {
                            $(this).removeClass(Validatemsg).addClass("txt");
                        }
                        break;
                    }
                case "DateTimeOrNull":
                ...
                case "isDateTimeMinute":
                    {
                        if (!isDateTimeMinute($(this).attr("value"), $(this).attr("eqvalue"))) {
                            Validatemsg = $(this).attr("err") + "必须为日期时间(分钟)格式！\n";
                            Validateflag = false;
                            ChangeCss($(this), Validatemsg); return false;
                        }
                        else 
                ...
                case "isDateTimeMinuteOrNull":
                    {
                        if (!isDateTimeMinuteOrNull($(this).attr("value"), $(this).attr("eqvalue"))) {
                            Validatemsg = $(this).attr("err") + "必须为日期时间(分钟)格式！\n";
                            Validateflag = false;
                            ChangeCss($(this), Validatemsg); return false;
                        }
                ...
                case "Age":
                    {
                        if (!isAge($(this).attr("value"), $(this).attr("eqvalue"))) {
                            Validatemsg = $(this).attr("err") + "年龄必须为两位小数！\n";
                            Validateflag = false;
                            ChangeCss($(this), Validatemsg); return false;
                        }
                        break;
